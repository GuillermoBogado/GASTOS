...
req GET "/balance?mes=$MES"
check "GET /balance → 200" 200 "$STATUS"
BAL_ING0="$(printf '%s' "$BODY" | jx 'd.ingresos')"; BAL_PREV0="$(printf '%s' "$BODY" | jx 'd.acumulado_ingresos-d.ingresos')"
...
seccion "GET /balance — cuánto te queda"
req GET "/balance?mes=$MES"
check "trae ingresos, gastos y acumulados numéricos" true "$(printf '%s' "$BODY" | jx '["ingresos","gastos","acumulado_ingresos","acumulado_gastos"].every(k=>typeof d[k]==="number")')"
BAL="$BODY"
resumen "&tipo=gasto";   check "los gastos del balance = total de gastos de /resumen" "$(printf '%s' "$BODY" | jx 'd.total')" "$(printf '%s' "$BAL" | jx 'd.gastos')"
resumen "&tipo=ingreso"; check "los ingresos del balance = total de ingresos de /resumen" "$(printf '%s' "$BODY" | jx 'd.total')" "$(printf '%s' "$BAL" | jx 'd.ingresos')"
BAL_PREV1="$(printf '%s' "$BAL" | jx 'd.acumulado_ingresos-d.ingresos')"
check "el ingreso de enero (2.500.000) suma al saldo acumulado y no al mes" 2500000 "$(node -e "console.log(Math.round(($BAL_PREV1-$BAL_PREV0)*100)/100)")"
check "el acumulado siempre incluye al mes" true "$(printf '%s' "$BAL" | jx 'd.acumulado_ingresos>=d.ingresos&&d.acumulado_gastos>=d.gastos')"
req GET "/balance?mes=$MES&cuenta=Tarjeta"; BAL_T="$BODY"
resumen "&tipo=gasto&cuenta=Tarjeta"
check "con cuenta=Tarjeta el balance coincide con /resumen de esa cuenta" "$(printf '%s' "$BODY" | jx 'd.total')" "$(printf '%s' "$BAL_T" | jx 'd.gastos')"
req GET "/balance?mes=2026-13";  check "balance con mes inválido → 400" 400 "$STATUS"
req GET "/balance?mes=2026-01";  check "balance de un mes pasado → 200" 200 "$STATUS"
check "…y muestra ese ingreso de enero" true "$(printf '%s' "$BODY" | jx 'd.ingresos>=2500000')"
